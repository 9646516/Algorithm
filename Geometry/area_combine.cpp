...
void pushup(int root) {
	if(tree[root].val)tree[root].sum=v[tree[root].r+1]-v[tree[root].l];
	else if(tree[root].r==tree[root].l)tree[root].sum= 0;
	else tree[root].sum=tree[root*2+1].sum+tree[root*2+2].sum;
}
